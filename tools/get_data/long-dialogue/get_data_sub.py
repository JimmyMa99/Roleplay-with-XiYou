        ...
        if i!=0:
            ...
            for sentence in conversation_list:
                ...
                if (tag+'：') not in sentence:
                    ...
                    input_sentence = copy.deepcopy(sentence[colon_index+1:])
                else:
                    colon_index = sentence.find("：")
                    if input_sentence:
                        single_conversation=copy.deepcopy(single_conversation_)
                        single_conversation['system']=tag
                        single_conversation['input']=input_sentence
...
def save_json(args,data,tag):
    with open((args.save_path+tag+'_sub.jsonl'), 'a', encoding='utf-8') as f:
        # for item in data:
        #     if item == []:
        #         continue
